...
export default async function PseoPage({ params }: PageProps) {
  ...
  return (
    <PseoPageLayout breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Directory', href: '/directory' }, { label: item.content.h1 }]} internalLinks={item.internal_links} callToAction={item.content.call_to_action} schemaJsonLd={item.schema.structured_data}>
      <PseoContentRenderer h1={item.content.h1} introduction={item.content.introduction} sections={item.content.sections} faq={item.content.faq} badges={['Directory', 'Find']} keyTakeaways={item.content.key_takeaways} featuredImages={item.featured_images} lastUpdated={item.last_updated} />
    </PseoPageLayout>
  );
}
